...
def batch_norm(x, is_training, epsilon=1e-5, decay=0.9, scope="batch_norm",
    ...
    with tf.device(parameter_update_device):
        ...
        # var =tf.layers.batch_normalization(x, momentum=decay, updates_collections=None, epsilon=epsilon,
        #                                    scale=True, is_training=is_training, scope=scope)

            var = tf.layers.batch_normalization(x, momentum=decay, epsilon=epsilon,
...
def layer_norm(x, scope="layer_norm",
    ...
    with tf.device(parameter_update_device):
        ...
        #var = tf.contrib.layers.layer_norm(x,scope=scope)
            var = tf.keras.layers.LayerNormalization(axis = -1)(x)
...
def conv2d(x,
    ...
    with tf.variable_scope(scope):
        ...
        if not weight_decay ==-1:
            this_weight_decay_result = tf.multiply(tf.nn.l2_loss(W), weight_decay, name='weight_decay')
            tf.add_to_collection(W.name.replace(':0',"-WeightDecay"), this_weight_decay_result)
            
            # if not name_prefix.find('/') == -1:
            #     tf.add_to_collection(name_prefix[0:name_prefix.find('/')] + '_weight_decay', this_weight_decay_result)
            # else:
            #     tf.add_to_collection(name_prefix + '_weight_decay', this_weight_decay_result)
...
def dilated_conv2d(x,
    ...
    with tf.variable_scope(scope):
        ...
        if weight_decay:
            this_weight_decay_result = tf.multiply(tf.nn.l2_loss(W), weight_decay_rate, name='weight_decay')
            tf.add_to_collection(W.name.replace(':0',"-WeightDecay"), this_weight_decay_result)
            # if not name_prefix.find('/') == -1:
...
def deconv2d(x, output_shape,
    ...
    with tf.variable_scope(scope):
        ...
        if not weight_decay==-1:
            ...
            tf.add_to_collection(W.name.replace(':0',"-WeightDecay"), this_weight_decay_result)
            # if not name_prefix.find('/') == -1:
            #     tf.add_to_collection(name_prefix[0:name_prefix.find('/')] + '_weight_decay', weight_decay_result)
            # else:
            #     tf.add_to_collection(name_prefix + '_weight_decay', weight_decay_result)
...
def desblock(input_x,output_filters,
             initializer,
             layer,sh,sw,kh,kw,batch_norm_used,is_training,
             weight_decay,weight_decay_rate,
             scope="resblock",
...
def fc(x,
    ...
    with tf.variable_scope(scope):
        ...
        if not weight_decay==-1:
            ...
            tf.add_to_collection(W.name.replace(':0',"-WeightDecay"), this_weight_decay_result)
            # if not name_prefix.find('/') == -1:
            #     tf.add_to_collection(name_prefix[0:name_prefix.find('/')] + '_weight_decay', _result_wd)
            # else:
            #     tf.add_to_collection(name_prefix + '_weight_decay', _result_wd)
